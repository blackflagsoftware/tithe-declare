...
func RouteVersionNotFoundError(route, version string) ApiError {
	return NewApiError(
		http.StatusNotFound,
		"Route Version Not Found",
		fmt.Sprintf("Route: %s does not support version: %s", route, version),
		false,
		nil,
	)
}
...
func MissingAcceptHeaderError(appName string) ApiError {
	return NewApiError(
		http.StatusBadRequest,
		"Missing Accept Header",
		"Required: 'accept' header with value: application/vnd."+appName+".v*+json",
		false,
		nil,
	)
}
...
func EmailPasswordComboError() ApiError {
	return NewApiError(
		...
		"The email/password entered was not valid, please try again",
		false,
		nil,
	)
}
