...
const titleText = ref<string>("Callback")
const id = ref<string>("")
const callbackUrl = ref<string>("")
const active = ref<boolean>(true)

function saveClick() {
	const callback = {callback_url: callbackUrl.value}
	emit("clickSubmit", callback)
	blankLocalRef()
}
...
watch(() => props.callback, (newCallback) => {
		if (newCallback) {
			console.log("document:", document)
			callbackUrl.value = newCallback.callback_url
		}
	},
	{ immediate: true }
)
			...
			<div class="relative p-4 w-full max-w-2xl max-h-full">
				<div class="relative bg-white dark:bg-gray-900 rounded-lg shadow dark:bg-black">
						...
						<div class="p-4 md:p-5 space-y-8 dark:bg-gray-900">
							<div>
								<label for="callback-url" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Callback</label>
								<input id="callback-url" type="text" v-model="callbackUrl" class="block p-2.5 max-w-screen-lg w-96 text-sm text-gray-900 bg-gray-200 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"></input>
							</div>
						</div>
