...
const titleText = ref<string>("Client")
const id = ref<string>("")
const name = ref<string>("")
const description = ref<string>("")
const homepageUrl = ref<string>("")

function saveClick() {
	const client = {id: id.value, name: name.value, description: description.value, homepage_url: homepageUrl.value}
	emit("clickSubmit", client)
	blankLocalRef()
}
...
watch(() => props.client, (newClient) => {
		if (newClient) {
			...
			description.value = newClient.description
			homepageUrl.value = newClient.homepage_url
		}
	},
	{ immediate: true }
)
			...
			<div class="relative p-4 w-full max-w-2xl max-h-full">
				<div class="relative bg-white dark:bg-gray-900 rounded-lg shadow dark:bg-black">
						...
						<div class="p-4 md:p-5 space-y-8 dark:bg-gray-900">
							...
							<div>
								<label for="homepageUrl" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Homepage Url</label>
								<input
								id="homepageUrl"
								type="text"
								v-model="homepageUrl"
